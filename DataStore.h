...
#define DL_STR const char *
// #define DL_T(quote) QStringLiteral(quote)
#define DL_T(quote) quote
...
private:
    ...
    QString errors;
    bool prepare(const QString &sql, QSqlQuery &query, int params_count, CValue *params[]);
    bool readRowData(QSqlQuery &query, int ret_values_count, CValue *ret_values[], DL_STR col_names[]);

public:

    const QString & getLastErrors() {
        return errors;
    }
...
public:
    std::list<T> list; // TODO: not tested yet
